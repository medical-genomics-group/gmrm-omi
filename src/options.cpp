...
void Options::read_command_line_options(int argc, char** argv) {
    ...
    for (int i=1; i<argc; ++i) {
        ...
        if (!strcmp(argv[i], "--bin-file")) {
            if (i == argc - 1) fail_if_last(argv, i);
            bin_file = argv[++i];
            ss << "--bin-file " << bin_file << "\n";
        }
        ...
        // List of phenotype files to read; comma separated if more than one.
            ...
            burn_in = (unsigned int) atoi(argv[++i]);
            ss << "--burn-in " << burn_in << "\n";
            ...
            if (i == argc - 1) fail_if_last(argv, i);
            if (atoi(argv[i + 1]) < 0) {
                std::cout << "FATAL  : cov-num --number of covariates has to be unsigned integer! (" << argv[i + 1] << " was passed)" << std::endl;
                exit(EXIT_FAILURE);
            }
...
// Check for minimal setup: a bin file + a dim file + phen file(s)
void Options::check_options() {
    if (get_bin_file() == "") {
        std::cout << "FATAL  : no bin file provided! Please use the --bin-file option." << std::endl;
        exit(EXIT_FAILURE);
    }
